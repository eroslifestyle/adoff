...
(function () {
  ...
  if (isVideoPlatform) {
    ...
    function maybeStripYtResp() {
      ...
      if (isProEnabled()) { stripAdObj(_ytResp); _ytStripped = true; }
    }
    ...
    window.fetch = function (input, init) {
      ...
      // È una richiesta player/next: SEMPRE intercettata, anche se il Pro non è
      // ancora confermato. injectNoAd best-effort se Pro già attivo alla richiesta.
      if (isProEnabled() && init && init.body) {
      ...
      return _origFetch.call(window, input, init).then(function (resp) {
        ...
        // GATE PRO AL MOMENTO-RISPOSTA: content.js conferma il Pro in modo
  ...
  function isProEnabled() {
    ...
    // CACHE SINCRONA: content.js setta l'attributo in modo ASINCRONO (dopo la
    // verifica del token firmato), quasi sempre DOPO che YouTube ha gia' letto
    // ytInitialPlayerResponse → lo strip degli ad perde la gara sul cold-load.
    // Per vincerla, content.js scrive anche un timestamp in localStorage (stessa
    // origine, condivisa col MAIN world): qui lo leggiamo in modo SINCRONO a
    // document_start, cosi' la prevenzione e' gia' attiva al primo accesso del
    // player. content.js cancella la cache quando NON e' Pro (no abuso passivo).
    try {
