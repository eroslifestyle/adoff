(function () {
  ...
  // TTL della cache sincrona YouTube (vedi blocco isPro): backstop breve,
  // rinfrescato a ogni caricamento Pro. Limita l'uso passivo se la cache resta.
  const YT_PRO_CACHE_TTL_MS = 6 * 60 * 60 * 1000;
  ...
  chrome.storage.local.get(["adoffEnabled", "adoffAdsBlocked", "adoffWhitelist", "adoffTrialEnd", "adoffTrialToken", "adoffTrialExpired", "adoffDeviceId", "adoffLicense", "adoffIntegrity"], async (result) => {
    ...
    // CACHE SINCRONA YOUTUBE: l'attributo qui sopra arriva troppo tardi per
    // vincere la gara con ytInitialPlayerResponse sul cold-load (questo gate e'
    // dietro storage.get + verifica token, entrambi async). Scriviamo un TTL
    // breve in localStorage (stessa origine, letto SINCRONO da stealth.js a
    // document_start) cosi' la PREVENZIONE degli ad vince gia' dal 2° load.
    // Si rinfresca a ogni load Pro e si auto-cancella se l'utente non e' piu'
    // Pro (no abuso passivo da trial scaduto).
    if (hostname.includes("youtube.com")) {
      try {
        if (isPro) {
          localStorage.setItem("__aoyt", String(Date.now() + YT_PRO_CACHE_TTL_MS));
        } else {
          localStorage.removeItem("__aoyt");
        }
      } catch (_) { /* localStorage puo' essere bloccato — non bloccante */ }
    }
  ...
  // ====== ANTI-ADBLOCK WALL KILLER (DOM-level, UNGATED) ======
  // Secondo layer oltre allo strip della response in stealth.js. Se YouTube
  // inietta il muro ("I blocchi degli annunci non sono consentiti") client-side
  // per una via che lo strip non copre (es. ytInitialData / detection script),
  // lo eliminiamo qui dal DOM. SEMPRE attivo (Free incluso): il blocco network
  // di AdOff e' sempre on → il muro va sempre soppresso. Il player resta in
  // pausa SOTTO il modale, quindi rimuovere popup + backdrop e fare play()
  // ripristina la riproduzione. Renderer noto uBO + euristica tag-"enforcement"
  // (sopravvive ai rinominamenti tipo bkaEnforcementMessageViewModel).
  function killAdblockWall() {
    let renderer = document.querySelector(
      "ytd-enforcement-message-view-model, ytd-enforcement-message-renderer"
    );
    if (!renderer) {
      ...
      if (pc) {
        ...
        for (const el of nodes) {
          if (/enforcement/i.test(el.localName || "")) { renderer = el; break; }
        }
      }
    }
    if (!renderer) return;
    const dialog = renderer.closest("tp-yt-paper-dialog") ||
                   renderer.closest("ytd-popup-container") || renderer;
    try { dialog.remove(); } catch (_) { /* ignore */ }
    const backdrops = document.querySelectorAll("tp-yt-iron-overlay-backdrop");
    for (const b of backdrops) { try { b.remove(); } catch (_) { /* ignore */ } }
    ...
    if (video && video.paused) { try { video.play(); } catch (_) { /* ignore */ } }
  }

  if (isVideoPlatform()) {
    // Backstop a intervallo per tutta la sessione (cheap: una querySelector).
    // La CSS in ads-hide.css rende il popup invisibile all'istante (no flash);
    // qui ne ripristiniamo la funzionalita' (backdrop + play) entro ~0.5s.
    setInterval(killAdblockWall, 500);
  }
