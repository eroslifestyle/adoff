(function () {
  ...
  // L'autorità del trial è il server (endpoint /trial → tabella D1). Il
  // background verifica il token firmato con la chiave pubblica: la scadenza non
  // è falsificabile via DevTools/storage. Vedi license-client.js (stessa logica).
  ...
  chrome.runtime.onInstalled.addListener(async (details) => {
    ...
    // Ancora/riconcilia il trial col server. Su "update" RIPRISTINA la scadenza
    // autorevole dal server anche se lo storage locale fosse azzerato → il
    // countdown non si resetta mai più tra un aggiornamento e l'altro.
    syncTrialBg();
    updateUninstallURL();
  ...
  function updateImaRules() {
    chrome.storage.local.get(
      ...
      // se assente (browser vecchio) degrada senza regressione.
  ...
  // Mantiene i filtri freschi senza aspettare la review dello store: il background
  // scarica un JSON firmato-per-origine da adoff.app e lo applica a runtime.
