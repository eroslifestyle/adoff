...
- [ ] 🔴 **Feed `2026.08.02.2` generato e verificato ma NON online**: `deploy-site.sh` si ferma sul gate i18n con **774 hard failures preesistenti** — 29 chiavi `notfound.*` e `about.*` usate nell'HTML ma assenti dalla matrice, che poi risultano mancanti in tutte le 14 lingue. Non è causato dal lavoro sul feed: il gate falliva già prima. Finché non si sanano quelle chiavi **nessun deploy del sito può partire**. In produzione resta `2026.08.02.1`. Backup: `sviluppo/archive/rules-feed-pre-generatore-20260802.json`.
