...
- [x] Sincronizzazione `origin/main` eseguita (2026-09-12, sessione post-checkpoint CP_20260912_1835): utente ha confermato esplicitamente "snapshot sanificato + force-push". Trovata e fixata 1 credenziale reale in più (password Postgres hardcoded in `sviluppo/ai-autopilot/n8n-workflows/scripts/setup-credentials.sh`, sostituita con env var `N8N_PG_PASSWORD`, commit `81c5b641`). Nuovo snapshot orfano creato da main locale (HEAD `81c5b641`, 2215 file, albero identico verificato con `git diff`), pushato con `--force` su origin/main: nuovo HEAD pubblico `ad0364be`. Main locale (493 commit, mai pushato direttamente) invariato.
...
**Aperto:**
- [x] **Sincronizzazione `origin/main` eseguita** (2026-09-12, turno successivo): utente ha confermato "sì, snapshot sanificato + force-push". Scan pre-pubblicazione (grep mirato, gitleaks non installato sul sistema) ha trovato 1 credenziale reale residua non coperta dagli audit precedenti (password Postgres hardcoded in setup-credentials.sh) — fixata prima del push (commit `81c5b641`). Nuovo snapshot orfano da main locale pushato con --force: origin/main ora a `ad0364be` (2215 file, storia singola). Dettaglio: vault `Memoria/progetti/AdOff/sessioni/adoff-remediation-sicurezza-3-round-2026-09-12.md`.
...
- NON ruotare N8N_ENCRYPTION_KEY senza un piano di re-encryption delle credenziali n8n esistenti
