...
1. **Network blocking** (`declarativeNetRequest`) — 180 regole `block`/`allow` (nessun `redirect`) bloccano richieste HTTP verso ad network, incluso il blocco di IMA SDK (`imasdk.googleapis.com`, rule 900). Le regole si dividono in **ads reali** (contate nel badge) e **tracking/analytics** (bloccati ma non contati, 23 rule IDs esclusi)
...
- `rules/adblock-rules.json` — 180 regole declarativeNetRequest (179 block incluso IMA SDK rule 900 + 1 allow GTM broadcaster rule 910; nessun redirect)
