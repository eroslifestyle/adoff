...
const SCN=[
 {slug:'recipe',url:'recipe-page-en.html',hb:'You just wanted the recipe.',ha:'Just the recipe. Zero ads.',type:'css'},
 {slug:'video', url:'video-page-en.html', hb:'Two ads before every video…',ha:'Press play. It just plays.',type:'video'},
 {slug:'wall',  url:'wall-page-en.html',  hb:'\u00abDisable your ad blocker to continue.\u00bb',ha:'With AdOff, the wall never appears.',type:'wall'},
];
function ov(a){const hk=a.hk,after=a.after,land=a.land;
  ...
  if(after){const g=document.createElement('div');g.className='ovl';g.textContent='✓ Blocked by AdOff';g.style.cssText=`position:fixed;top:${land?70:86}px;left:50%;transform:translateX(-50%);z-index:99999;background:#4ade80;color:#0a0a1a;font-family:Arial;font-weight:800;font-size:${bd}px;padding:9px 18px;border-radius:30px;box-shadow:0 6px 24px #0008`;document.body.appendChild(g);}
}
...
for(const land of [false,true]){
  ...
  for(const sc of SCN){
    ...
    await p.screenshot({path:`${OUT}/${sc.slug}_${tag}_en_before.png`}); await p.close();
  }
  await ctx.close();
  // AFTER (con AdOff)
  ...
  for(const sc of SCN){
    ...
    await p.screenshot({path:`${OUT}/${sc.slug}_${tag}_en_after.png`}); await p.close();
  }
  await ctx.close();
