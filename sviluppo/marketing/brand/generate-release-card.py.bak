...
# Riga versione
draw.text((LEFT, 100), 'VERSION 3.5.62', font=font_small, fill=COLORS['viola_tenue'])

# Titolo su due righe
y_title = 145
draw.text((LEFT, y_title), 'Protection now reaches', font=font_title, fill=COLORS['bianco'])
draw.text((LEFT, y_title + 55), 'inside embedded players', font=font_title, fill=COLORS['bianco'])

# Elenco con pallini verdi
y_list = 265
items = [
    'Ad-free video inside third-party players',
    'Pop-under blocking that no longer depends on the site',
    'Catches ad networks that keep changing domain'
]
for item in items:
...
# Salva
output_path = 'sviluppo/marketing/BRAND-HUB/3-IMMAGINI-SOCIAL/release-3.5.62__1200x628__en.png'
