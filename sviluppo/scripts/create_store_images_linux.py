...
def create_screenshot3():
    ...
    right_features = [
        ...
        ("Multi-language", "15 languages: EN, IT, DE, FR..."),
...
# === SCREENSHOT 4: Video ad neutralization (1280x800) ===
def create_screenshot4():
    img = Image.new("RGB", (1280, 800), BG_DARK)
    draw = ImageDraw.Draw(img)
    draw.text((640, 40), "Video ads? Gone.", fill=WHITE, font=font(46, True), anchor="mt")
    draw.text((640, 100), "Pre-roll, mid-roll and overlays neutralized at the source", fill=GRAY, font=font_light(22), anchor="mt")

    # Left: WITHOUT (red player)
    lx, ly, lw, lh = 80, 180, 540, 440
    draw_rounded_rect(draw, (lx, ly, lx + lw, ly + lh), 16, (40, 22, 22))
    draw.text((lx + lw // 2, ly + 24), "WITHOUT AdOff", fill=RED, font=font(22, True), anchor="mt")
    draw_rounded_rect(draw, (lx + 40, ly + 80, lx + lw - 40, ly + 300), 12, (20, 12, 12))
    draw.text(((lx + lw // 2), ly + 150), "▶  ADVERTISEMENT", fill=RED, font=font(26, True), anchor="mt")
    draw.text(((lx + lw // 2), ly + 195), "Skip in 5... 4... 3...", fill=(200, 120, 120), font=font(18), anchor="mt")
    draw.text((lx + lw // 2, ly + 340), "Ad break  ·  countdown  ·  another ad", fill=(200, 120, 120), font=font(16), anchor="mt")
    draw.text((lx + lw // 2, ly + 390), "Slow start, interruptions", fill=RED, font=font(16), anchor="mt")

    # Right: WITH (green player)
    rx = 660
    draw_rounded_rect(draw, (rx, ly, rx + lw, ly + lh), 16, (18, 36, 24))
    draw.text((rx + lw // 2, ly + 24), "WITH AdOff", fill=GREEN, font=font(22, True), anchor="mt")
    draw_rounded_rect(draw, (rx + 40, ly + 80, rx + lw - 40, ly + 300), 12, (12, 22, 16))
    draw.text((rx + lw // 2, ly + 150), "▶  Your video", fill=GREEN, font=font(26, True), anchor="mt")
    draw.text((rx + lw // 2, ly + 195), "Plays instantly · No ads", fill=(150, 210, 170), font=font(18), anchor="mt")
    draw.text((rx + lw // 2, ly + 340), "Zero pre-roll  ·  zero mid-roll", fill=(150, 210, 170), font=font(16), anchor="mt")
    draw.text((rx + lw // 2, ly + 390), "Watch in peace", fill=GREEN, font=font(16), anchor="mt")

    icon_b = icon_orig.resize((44, 44), Image.LANCZOS)
    img.paste(icon_b, (470, 660), icon_b)
    draw.text((524, 672), "Works on streaming & video platforms  —  Pro / Trial", fill=LIGHT_GRAY, font=font(20, True))
    img.save(os.path.join(OUT_DIR, "screenshot4_video.png"), "PNG")
    print("Screenshot 4 (video) saved")


# === SCREENSHOT 5: Stealth / anti-adblock (1280x800) ===
def create_screenshot5():
    img = Image.new("RGB", (1280, 800), BG_DARK)
    draw = ImageDraw.Draw(img)
    icon_t = icon_orig.resize((64, 64), Image.LANCZOS)
    img.paste(icon_t, (608, 30), icon_t)
    draw.text((640, 110), "Invisible by design", fill=WHITE, font=font(46, True), anchor="mt")
    draw.text((640, 168), "AdOff stays undetected, even on sites that fight ad blockers", fill=GRAY, font=font_light(22), anchor="mt")

    # Center: the wall that never appears
    cx, cy, cw, ch = 340, 240, 600, 180
    draw_rounded_rect(draw, (cx, cy, cx + cw, cy + ch), 16, (40, 22, 22))
    draw.text((cx + cw // 2, cy + 45), "“Please disable your ad blocker”", fill=(200, 120, 120), font=font(24, True), anchor="mt")
    draw.text((cx + cw // 2, cy + 100), "✘  never appears with AdOff", fill=GREEN, font=font(22, True), anchor="mt")
    # diagonal strike
    draw.line((cx + 30, cy + 30, cx + cw - 30, cy + ch - 30), fill=RED, width=4)

    feats = [
        ("Variable spoofing", "Fakes the JS variables sites use to detect blockers", GREEN),
        ("Fetch / XHR interception", "Neutralizes anti-adblock test requests", BLUE),
        ("Bait spoofing", "Simulates the decoy elements sites expect", PURPLE),
        ("MAIN-world operation", "Masks every trace of an active extension", YELLOW),
    ]
    gy = 470
    for i, (t, d, c) in enumerate(feats):
        ...
        fx = 120 + col * 560
        fy = gy + row * 120
        draw_rounded_rect(draw, (fx, fy, fx + 520, fy + 95), 14, BG_CARD)
        draw.ellipse((fx + 24, fy + 38, fx + 40, fy + 54), fill=c)
        draw.text((fx + 56, fy + 24), t, fill=WHITE, font=font(20, True))
        draw.text((fx + 56, fy + 54), d, fill=GRAY, font=font(14))
    img.save(os.path.join(OUT_DIR, "screenshot5_stealth.png"), "PNG")
    print("Screenshot 5 (stealth) saved")


# === SCREENSHOT 6: Privacy & honesty (1280x800) ===
def create_screenshot6():
    img = Image.new("RGB", (1280, 800), BG_DARK)
    draw = ImageDraw.Draw(img)
    icon_t = icon_orig.resize((64, 64), Image.LANCZOS)
    img.paste(icon_t, (608, 30), icon_t)
    draw.text((640, 110), "Private. Honest. Yours.", fill=WHITE, font=font(46, True), anchor="mt")
    draw.text((640, 168), "On-device filtering, zero logs, real numbers", fill=GRAY, font=font_light(22), anchor="mt")

    cards = [
        ("\U0001F512", "Zero-Log Policy", "No browsing history, no page content, nothing sent to our servers."),
        ("\U0001F310", "100% On-Device", "All filtering runs locally. The only call is Pro license validation."),
        ("\U0001F6E0", "Built in Public", "One founder, constant updates. No fake reviews, no inflated numbers."),
    ]
    cw = 360
    for i, (emoji, title, desc) in enumerate(cards):
        cx = 80 + i * (cw + 40)
        cy = 250
        draw_rounded_rect(draw, (cx, cy, cx + cw, cy + 320), 16, BG_CARD)
        draw.text((cx + cw // 2, cy + 40), title, fill=WHITE, font=font(24, True), anchor="mt")
        draw.line((cx + 40, cy + 90, cx + cw - 40, cy + 90), fill=DARK_GRAY, width=1)
        # wrap desc
        words = desc.split()
        line = ""
        ty = cy + 120
        for w in words:
            test = (line + " " + w).strip()
            if draw.textlength(test, font=font(16)) > cw - 60:
                draw.text((cx + cw // 2, ty), line, fill=GRAY, font=font(16), anchor="mt")
                ty += 30
                line = w
            else:
                line = test
    ...
    # bottom badges
    by = 630
    badges = ["138 rules", "15 languages", "30-day trial", "cancel anytime", "30-day refund"]
    bx = 110
    for b in badges:
        w = draw.textlength(b, font=font(18, True)) + 44
        draw_rounded_rect(draw, (bx, by, bx + w, by + 56), 28, BG_CARD)
        draw.text((bx + w // 2, by + 16), b, fill=LIGHT_GRAY, font=font(18, True), anchor="mt")
        bx += w + 20
    img.save(os.path.join(OUT_DIR, "screenshot6_privacy.png"), "PNG")
    print("Screenshot 6 (privacy) saved")
